...
### Added
- **ChannelEngine — `orders_api` mode**: `ChannelEngineClient` now accepts an `orders_api: bool = False` constructor parameter. When `True`, all data is fetched from `GET /v2/orders` instead of `GET /v2/shipments`. This is required for tenants (e.g. Villeroy & Boch) whose API key lacks access to the shipments endpoint.
